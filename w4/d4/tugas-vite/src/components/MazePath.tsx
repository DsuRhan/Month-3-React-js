...
function useGenerateMaze(size = 21) {
  ...
  const regenerate = useCallback(() => setMaze(generateMaze()), [generateMaze])
...
const MazePath: FC<MazePathProps> = ({ onWin, onLose }) => {
  ...
  const [pathTrail, setPathTrail] = useState<Set<string>>(new Set())

  const handleStart = () => {
    setStarted(true)
    setIsPlaying(true)
    setPathTrail(new Set())
  }
  ...
  const handlePointerMove = (x: number, y: number) => {
    ...
    setHoveredCell([x, y])
    setPathTrail(prev => new Set(prev).add(`${x}-${y}`))
      ...
      >
        {maze.map((row, y) =>
          row.map((cell, x) => {
            ...
            const isTrail = pathTrail.has(`${x}-${y}`)
            ...
            return (
              <div
                ...
                onPointerMove={(e) => {
                  if (e.pressure > 0 || e.buttons > 0) handlePointerMove(x, y)
                }}
                className={`w-5 h-5 ${bgColor} transition-colors duration-75 ${
